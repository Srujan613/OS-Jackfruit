...
/* ── timer callback (runs every CHECK_INTERVAL_SEC seconds) ─────────────── */
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        /* Process exited — clean up the entry */
        if (rss < 0) {
        ...
        /* Soft limit: warn once, keep monitoring */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    /* Null-terminate container_id defensively in case userspace didn't */
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            pr_warn("[container_monitor] Rejected: soft > hard for pid=%d\n", req.pid);
            return -EINVAL;
        }

        /* Validate: limits must be non-zero */
        if (req.soft_limit_bytes == 0 || req.hard_limit_bytes == 0) {
            pr_warn("[container_monitor] Rejected: zero limit for pid=%d\n", req.pid);
            return -EINVAL;
        }
        ...
        /* FIX: use strscpy (replaces deprecated strncpy in kernel 6.x) */
        strscpy(entry->container_id, req.container_id, MONITOR_NAME_LEN);

        mutex_lock(&monitored_lock);

        /*
         * FIX: check for duplicate registration (same pid + container_id).
         * Without this, re-registering the same process leaks the old entry
         * and doubles the monitoring, causing spurious kills.
         */
        list_for_each_entry(existing, &monitored_list, list) {
            if (existing->pid == req.pid &&
                strncmp(existing->container_id, req.container_id,
                        MONITOR_NAME_LEN) == 0) {
                mutex_unlock(&monitored_lock);
                kfree(entry);
                pr_warn("[container_monitor] Duplicate registration for pid=%d\n",
                        req.pid);
                return -EEXIST;
            }
        }
    ...
    /* ── UNREGISTER ── */
    {
        ...
        pr_info("[container_monitor] Unregister container=%s pid=%d\n",
                req.container_id, req.pid);
...
/* FIX: must be const — kernel enforces this since 5.x */
